...
- **Alarm-Feedback:** läuft ein Timer ab, blinkt an der Status-Position im Kartenband
  ...
  pulsiert dort ohnehin). Die Uhr ist REIN ABGELEITET aus den Fakten (kein eigener
  Zustand): abgelaufener gesetzter Timer (Tombstone) oder abgelaufenes Plan-Gate —
  übersteht Reloads. Dazu Ton
  ...
  TTS); danach liest die vorhandene TTS-Logik den Text vor.
### Karte (allgemein)
```
┌──────────────────────────────────┐
...
- **Sichtbarkeit steht in der URL:** `?modal=chat`, `?modal=ingredients`, `?modal=config` —
  **genau ein Modal gleichzeitig:** Öffnen schließt die anderen;
  **Ansicht: `?view=now`** (Jetzt-Queue allein), **`?view=flow&flow=<id>`** (Flow-Detail-View),
  Default = Übersicht —
