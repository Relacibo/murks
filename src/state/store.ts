...
const DEFAULT_SYSTEM_PROMPT = [
  ...
  'Schritte haben stabile ids — referenziere Abhängigkeiten immer über step_id. Du darfst Flows ad-hoc umbauen: Schritte einfügen (after_step_id), umbenennen (update_step), löschen (delete_step), teilen (split_step), Flows umbenennen (update_flow) oder löschen (delete_flow). read_step liest die Step-Description per TTS vor (flow_id + step_id) — nutze es wenn der Nutzer fragt was er als nächstes tun soll, und antworte dann nur „OK.". show_step zeigt dem Nutzer gezielt einen Schritt (Fokus + Puls).',
