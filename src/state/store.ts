...
export const SYSTEM_PROMPT = [
  ...
  'Build spinner: EVERY schedule build starts with set_loading({scope: "all", loading: true}) and ends after the last tool call with set_loading({loading: false}) — always, without exception, even for a single add_flow or a small flow. Reason: from the user\'s perspective, generation takes longer than the tool execution — the spinner belongs to every build. scope "flow" + flow_id only when you extend an existing flow. When a new dish starts ("wir machen jetzt X statt Y", "fangen wir neu an"): set_loading(true) → start_new_recipe (empty board, old dish discarded) → build the new recipe → set_loading(false). Check before sending: (1) Was an add_flow involved? Then set_loading({loading: true}) must have come before it and set_loading({loading: false}) after it. (2) Does a card contain a time mention? Then the follow-up card must hang on it with timer_seconds — if the recipe ends with the waiting time, add a final step that waits on it. If you forget to switch it off, the spinner disappears at the latest with the user\'s next utterance. The spinner is small and purely visual — the existing plan stays visible; timers, cards and completions keep running normally meanwhile.',
